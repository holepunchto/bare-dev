...
module.exports = async function bundle (entry, opts = {}) {
  ...
  switch (format) {
    case 'bundle': {
      ...
      if (obfuscate) {
        let pathValidator = () => true
        try {
          const content = await fs.readFile(path.join(process.cwd(), 'package.json'))
          const pkg = JSON.parse(content.toString())
          if (Array.isArray(pkg.obfs)) {
            const obfsPaths = pkg.obfs
            pathValidator = (inputPath) => {
              return obfsPaths.findIndex(excludedPath => {
                return inputPath.indexOf(excludedPath) >= 0
              }) >= 0
            }
          }
        } catch {
        ...
        bundle.map((data, file) => {
          ...
          const result = obfs(data.toString('utf8'))
          return Buffer.from(result.getObfuscatedCode(), 'utf8')
        })
      }
