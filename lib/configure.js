...
module.exports = function configure (opts = {}) {
  ...
  if (generator && isMultiConfigurationGenerator(generator)) {
  ...
  if (platform === 'android') {
    ...
    const sdk = require('./android/shared/sdk').path

    if (ndk) ndk = path.join(sdk, 'ndk', ndk)
    else {
      const candidates = glob.globSync(path.join(sdk, 'ndk', '*'), {
        windowsPathsNoEscape: true
      })

      if (!candidates.length) throw new Error('no NDK installed')

      ndk = candidates[0]
    }

    args.push(`-DCMAKE_ANDROID_NDK=${ndk}`)

    if (api) args.push(`-DCMAKE_ANDROID_API=${api}`)
  }
...
function isMultiConfigurationGenerator (generator) {
