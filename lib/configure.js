...
module.exports = function configure (opts = {}) {
  ...
  if (toolset && supportsToolsetSpecification(generator)) {
    args.push(`-DCMAKE_GENERATOR_TOOLSET=${toolset}`)
  }

  if (toolchain) {
    args.push(`-DCMAKE_TOOLCHAIN_FILE=${path.resolve(cwd, toolchain)}`)
  }
  ...
  if (supportsMultiConfiguration(generator)) {
  ...
  if (platform !== process.platform || arch !== process.arch) {
    ...
    } else if (supportsPlatformSpecification(generator)) {
